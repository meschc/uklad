  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative flex max-h-[88vh] w-full max-w-md animate-scale-in flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
          ...
          <Block label={t("addr.levels")}>
            ...
            <label className="flex cursor-pointer items-start gap-2.5">
              <input
                ...
                checked={cfg.useRows}
                onChange={(e) =>
                  updateAddressing({ useRows: e.target.checked, configured: true })
                }
                className="mt-0.5 size-4 accent-primary"
              />
              <span>
              ...
              {rows.length === 0 ? (
                ...
                <>
                  <p className="mb-2 text-xs">
                    {t("addr.rowsFound", { n: rows.length })}
                  </p>
            ...
            <div className="flex items-center gap-0.5 rounded-lg bg-muted p-0.5">
              {(
                ...
                <button
                  key={v}
                  onClick={() =>
                    updateAddressing({ shelfOrder: v, configured: true })
                  }
                  className={cn(
            ...
            <div className="flex items-center gap-0.5 rounded-lg bg-muted p-0.5">
              {["-", ".", "/"].map((sep) => (
                <button
                  key={sep}
                  onClick={() =>
                    updateAddressing({ separator: sep, configured: true })
                  }
                  className={cn(
