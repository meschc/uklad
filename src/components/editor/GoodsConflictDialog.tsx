      ...
      {conflict ? (
        <>
          ...
          <div className="flex flex-col gap-2 px-5 py-4">
            ...
            <Button size="sm" variant="outline" onClick={() => resolve("unplace")}>
...
function ResultView({
  ...
  return (
    <>
      <div
        className={
          ok ? "flex gap-3 px-5 py-4" : "hazard-stripes flex gap-3 bg-amber-500/10 px-5 py-4"
        }
      >
        <div
          className={
...
function lostPhrase(t: TFunc, n: number): string {
  if (t.lang === "en") {
    return n === 1 ? "1 product will lose its spot" : `${n} products will lose their spots`;
  }
  const d = n % 10;
  const dd = n % 100;
  const one = d === 1 && dd !== 11;
