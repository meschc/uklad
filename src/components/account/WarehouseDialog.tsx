...
export function WarehouseDialog({
  ...
  const [latText, setLatText] = useState(
    warehouse?.lat != null ? String(warehouse.lat) : "",
  );
  const [lngText, setLngText] = useState(
    warehouse?.lng != null ? String(warehouse.lng) : "",
  );
  const [rateText, setRateText] = useState(
    warehouse?.storageRatePerCell != null
      ? String(warehouse.storageRatePerCell)
      : "",
  );
  ...
  const lat =
    latText.trim() && !Number.isNaN(Number(latText)) ? Number(latText) : undefined;
  const lng =
    lngText.trim() && !Number.isNaN(Number(lngText)) ? Number(lngText) : undefined;
  ...
  const rate =
    rateText.trim() && !Number.isNaN(Number(rateText))
      ? Math.max(0, Number(rateText))
      : undefined;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative flex max-h-[90vh] w-full max-w-lg animate-scale-in flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl">
